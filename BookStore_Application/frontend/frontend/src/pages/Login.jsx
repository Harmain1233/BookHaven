...
const LogIn = () => {
  ...
  const Submit = async (e) => {
    ...
    try {
      ...
      if (error.response) {
        ...
        toast.error(error.response.data.message); // Show toast for server error
      } else if (error.request) {
        // The request was made but no response was received
        toast.error("No response from the server. Please try again later."); // Show toast for no response
      } else {
        // Other errors, such as in setting up the request
        toast.error(`Error: ${error.message}`); // Show toast for generic error
      }
    }
  };
  ...
  return (
    ...
    >
      ...
      <ToastContainer /> {/* Add the ToastContainer to display the notifications */}
    </div>
  );
};
